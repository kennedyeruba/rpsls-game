// SPDX-License-Identifier: GPL-3.0Ω
...
contract RPS{
    ...
    event Moved(address player); //Emitted when player moves
    event Revealed(uint8 outcome); //Emitted when j1 reveals move
    event TimedOut(address winner); //Emitted when either player times out
